...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  ...
  const outerShadow = typeof shadow === 'string' ? shadow
    : shadow ? '0 12px 30px rgba(0,0,0,0.35), 0 2px 6px rgba(0,0,0,0.22)' : 'none';
  ...
  return (
    <div className={className} style={{ boxSizing: 'border-box', display: 'inline-block', transform: `scale(${scale})`, transformOrigin: 'top left', ...style }}>
      <div aria-label={`iPhone mockup (${model})`} style={{ width: outerWidth, height: outerHeight, borderRadius: outerRadius, background: frameGradient, padding: resolvedBezel, boxSizing: 'border-box', boxShadow: outerShadow, position: 'relative', overflow: 'hidden', ...frameStyle }}>
        <div style={{ width: '100%', height: '100%', borderRadius: resolvedRadius, position: 'relative', overflow: 'hidden', background: screenBg, boxShadow: innerShadowCss, ...screenStyle }}>
          {wallpaper && (
            <div aria-hidden style={{ position: 'absolute', inset: 0, backgroundImage: `url(${wallpaper})`, backgroundSize: wallpaperFit, backgroundPosition: wallpaperPosition, backgroundRepeat: 'no-repeat', zIndex: 0 }} />
          )}
